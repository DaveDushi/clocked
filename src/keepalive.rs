//! "Keep clocked running": a per-user Scheduled Task that relaunches the app at
//! logon **and on workstation unlock**, so quitting or locking doesn't leave you
//! untracked. The unlock trigger isn't expressible through `schtasks` flags, so
//! we import a task XML. With the app's single-instance guard and the task's
//! `IgnoreNew` policy, the task is a no-op whenever clocked is already running.
